...
def resource_path(relative_path):
    ...
    try:
        ...
        # For normal execution, use the directory of the current script
...
def normalize_multivalues(raw):
    # Return NaN if raw is NaN or not a string
    ...
    for prefix, values, sign in matches:
        # Skip 'MY' tokens for current or next year
        if prefix == "MY":
            try:
                year_values = [int(v.strip()) for v in values.split(",") if v.strip().isdigit()]
                if any(y in [current_year, next_year] for y in year_values):
                    continue
            except Exception:
                continue
...
def extract_and_save_structured_data(df_61, mapping_file_path, df_griglia):
    ...
    try:
        ...
        multivalues_col = next(
            (col for col in df_61.columns if str(col).strip().lower() == "multivalues"), None
        )
        ...
        # Process each row using apply (more efficient than iterrows)
        def process_row(row):
            raw = str(row.get(multivalues_col) or "")  # Handle missing multivalues
            ...
            for token in tokens:
                ...
                # Get packet names
                packet_ita = str(mapping_dict_ita.get(token_base, token_base)).strip()
                packet_eng = str(mapping_dict_eng.get(token_base, token_base)).strip()
            ...
            # Step 2: If no match found with Italian, fallback to English
                if temp_df.index.empty:
                    ...
                    packet_references.add(packet_ita)


                griglia_multis = set()
                for mv in temp_df["Multivalues"]:
                    if pd.notna(mv):
                        griglia_multis.update(v.lower().strip().replace(" ", "") for v in str(mv).split(","))
                ...
                for t in tokens:
                    ...
                    s = t[-1]
                    base = t[:-1].strip()
                    if packet_ita in packet_references:
                        trans = mapping_dict_resp1.get(base, base)
                    else:
                        trans = mapping_dict_resp2.get(base, base)
                ...
                if sign == "+":
                    ...
                    for mv in griglia_multis:
                        if mv not in translated_tokens_lower:
                            excluded.add(mv)
                elif sign == "-":
                    for mv in translated_tokens_from_input:
                        excluded.add(str(mv).lower().replace(" ", "").strip())
                    for mv in griglia_multis:
                        if mv.lower() not in translated_tokens_lower:
                            included.add(str(mv).lower().replace(" ", "").strip())
    ...
    df_61 = map_multi_excluded_to_griglia(df_61, df_griglia, df_mapping)  # Pass df_mapping
    ...
    if all(col in df_61.columns for col in ["included", "Code_included", "multi_included_min_volume"]):
        ...
        # Only for rows that meet the condition
        df_61.loc[mask_multi_include_condition, "Final_Volume_include"] = np.maximum(
            df_61.loc[mask_multi_include_condition, "Code_included"].fillna(0),
            df_61.loc[mask_multi_include_condition, "multi_included_min_volume"].fillna(0)
        ).astype(float)
        ...
        if all(col in df_61.columns for col in ["excluded", "Code_excluded", "volume_Head"]):
            fallback_mask = (
                ...
                (
                    (df_61["Multi_included"].astype(str).str.strip().str.lower().isin(["none", ""])) |
                    (df_61["Multi_included"].fillna("").astype(str).str.strip() == "")
                ) &
                (
    ...
    final_columns = ["Modelo", "PN", "Plant", "multivalues", "included","Multi_excluded", "excluded","Unique_Key", "VolumeTT"]
    ...
    all_empty_mask = (
        ((df_61["multivalues"].fillna("").astype(str).str.strip().str.lower() == "") |(df_61["multivalues"].fillna("").astype(str).str.strip().str.lower() == "none") | (
            (df_61["multivalues"].fillna("").str.strip() != "") &
            (df_61["Multi_excluded"].fillna("").str.strip() == "")
        ))  
    ...
    df_61.drop(columns=["Multi_excluded"], inplace=True)
...
def map_multi_excluded_to_griglia(df_flattened, griglia_path, df_mapping):
    ...
    griglia["Model_cleaned"] = griglia["Model"].astype(str).str.strip()
    griglia["Plant_cleaned"] = griglia["Plant"].astype(str).str.strip()
    griglia["Code_cleaned"] = griglia["Code"].astype(str).str.strip().str.lower()
    griglia["Multivalues_cleaned"] = griglia["Multivalues"].astype(str).str.lower()
    griglia["Packet"] = griglia["Packet"].astype(str).str.strip()
    griglia["SINCOM"] = griglia["SINCOM"].astype(str).str.strip()

    # Explode multivalues for token matching
    griglia["Multivalue_token"] = griglia["Multivalues_cleaned"].str.split(",").apply(lambda tokens: [t.strip() for t in tokens])
    griglia_exploded = griglia.explode("Multivalue_token").reset_index(drop=True)

    # Clean mapping dataframe columns (if needed)
    df_mapping["MultiValues_clean"] = df_mapping["MultiValues"].astype(str).str.strip().str.lower()
    df_mapping["Griglia_Italiano_clean"] = df_mapping["Griglia Italiano"].astype(str).str.strip()
    df_mapping["Griglia_Inglês_clean"] = df_mapping["Griglia Inglês"].astype(str).str.strip()

    # Clean base dataframe columns
    df_flattened["Modelo"] = df_flattened["Modelo"].astype(str).str.strip()
    df_flattened["Plant"] = df_flattened["Plant"].astype(str).str.strip()
    df_flattened["SINCOM"] = df_flattened["SINCOM"].astype(str).str.strip()
    df_flattened["Multi_excluded"] = df_flattened["Multi_excluded"].fillna("").astype(str).str.strip()
    df_flattened["Multi_excluded_packets"] = df_flattened["Multi_excluded_packets"].fillna("").astype(str).str.strip()

    # Prepare result column
    df_flattened["multi_excluded_max_volume"] = 0

    def apply_row(row):
        model = row["Modelo"]
        plant = row["Plant"]
        sincom = row["SINCOM"]
        multi_excluded = row["Multi_excluded"]
        multi_excluded_tokens = [t.strip().replace(" ", "").lower() for t in multi_excluded.split(",") if t.strip()]
        multi_excluded_packets = [p.strip() for p in row["Multi_excluded_packets"].split(",") if p.strip()] if row["Multi_excluded_packets"] else []

        if not multi_excluded_tokens:
            return 0

        filtered = griglia_exploded[
            (griglia_exploded["Model_cleaned"] == model) &
            (griglia_exploded["Plant_cleaned"] == plant)
        ]

        if multi_excluded_packets:
            filtered = filtered[filtered["Packet"].isin(multi_excluded_packets)]

        filtered = filtered[filtered["SINCOM"] == sincom]

        # Normalize multivalue tokens by removing spaces and lowercasing
        ...
        if any(token in multivalue_tokens_in_filtered for token in multi_excluded_tokens):
            numeric_volumes = []
            for val in filtered["Volume Head"]:
                try:
                    numeric_volumes.append(float(val))
                except (ValueError, TypeError):
                    continue
    ...
    # Apply only on relevant rows
    mask = df_flattened["Multi_excluded"].str.lower().isin(["nan", "none", ""]) == False
    df_flattened.loc[mask, "multi_excluded_max_volume"] = df_flattened.loc[mask].apply(apply_row, axis=1)

    print("✅ Updated with 'multi_excluded_max_volume' using SINCOM + token existence check.")
    return df_flattened
